...
public class TestMDPParallelVarieCharge {
	...
	public static void explorer() throws IOException {
		...
		while(temps<100000) {
			...
			while (itAG.hasNext()) {
				...
				if(temps>10000) MDP.greedyEpsilon=0.1;
				...
				double tauxSurcharge=agent.cloud.nbRessourceNonDispo()/(nbRajout+agent.cloud.nbRessourceTotal());
				//System.out.println("3");
				
				// tenir compte du taux de surcharge pour décider si les requetes sont acceptées
				int nbRequetes=0;
				System.out.println(temps+" "+agent.mdp.etatEnCours.heureJournee);
