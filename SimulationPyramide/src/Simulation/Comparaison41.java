...
public class Comparaison41 {
	public static void main(String[] args) throws IOException{
		...
		Requete req=new Requete(1,25);
	    req.rajouterJob(new Job(req,3,1,3,2,4*2,4*2,4*2,4*1,4*1,4*1));
	    req.rajouterJob(new Job(req,2,2,2,2,4*1,4*1,4*1,4*1,4*1,4*1));
		...
		req=new Requete(1.5,20);
	    req.rajouterJob(new Job(req,3,2,3,3,4*2,4*2,4*2,4*2,4*2,4*2));
	    req.rajouterJob(new Job(req,3,1,2,3,4*2,4*2,4*2,4*1,4*1,4*1));
	    req.rajouterJob(new Job(req,2,2,2,2,4*1,4*1,4*1,4*1,4*1,4*1));
	    req.jobFinal=req.listeJobs.get(req.listeJobs.size()-1);
	    for(int j=0;j<req.nbJobs();j++){
	    	for(int k=0;k<req.nbJobs();k++){
		...
		req=new Requete(2,20);
	    req.rajouterJob(new Job(req,3,2,3,3,4*2,4*2,4*2,4*2,4*2,4*2));
	    req.rajouterJob(new Job(req,3,1,3,2,4*1,4*1,4*1,4*1,4*1,4*1));
	    req.rajouterJob(new Job(req,2,2,2,2,4*1,4*1,4*1,4*1,4*1,4*1));
	    req.jobFinal=req.listeJobs.get(req.listeJobs.size()-1);
	    for(int j=0;j<req.nbJobs();j++){
	    	for(int k=0;k<req.nbJobs();k++){
		...
		if(v==1){
		VariablesGlobales.writer_gbrt=new FileWriter("comp21-gbrt.csv",true);
		Gantt gantt;
	    ...
	    VariablesGlobales.writer_gmpt=new FileWriter("comp21-gmpt.csv",true);
		Gantt gantt;
	    ...
	    VariablesGlobales.writer_gmpm=new FileWriter("comp21-gmpm.csv",true);
		Gantt gantt;
	    ...
	    VariablesGlobales.writer_pl2p=new FileWriter("comp21-pl2p.csv",true);
	    Cout cout=new Cout();
		Gantt gantt;
