...
public class ModeleUnePhaseGLPKV2  implements GlpkCallbackListener{
	Cloud cloud;
	int nbJobs;
	int T;
	int[] nbMapTasks;
	...
	public ModeleUnePhaseGLPKV2(Cloud cloud,Cout cout){
		////////////////////////
		this.cout=cout;
		this.cloud=cloud;
		...
		try{
          ...
          for(int i=0;i<nbJobs;i++){
            ...
            if (ret == 0 || this.bonneSolutionTrouve) {
        		...
        		double coutPenalites=0;
                for(int i=0;i<nbJobs;i++){
        			for(int t=0;t<T;t++){
        				if(t>D[i]-Tr[i]){
        					coutPenalites=coutPenalites+P[i]*GLPK.glp_mip_col_val(lp, getRIndex(i, t));
        				}
        			}
                }
        		...
        		System.out.println("Cout Penalites : "+coutPenalites);
        		...
        		this.cout.coutPenalite=coutPenalites;
	...
	public void callback(glp_tree arg0) {
		...
		if (reason == GLPKConstants.GLP_IBINGO || reason==GLPKConstants.GLP_IHEUR) {
            ...
            if(GLPK.glp_ios_mip_gap(arg0) <= 0.2 || this.now-this.start>1200000){
        	//if(this.now-this.start>3600000){
