...
public class ModelePlacementGLPKV7  implements GlpkCallbackListener{
	Cloud cloud;
	int nbJobs;
	int T;
	int[] nbMapTasks;
	...
	int indexDebW;
	double minVW=0;
	double maxVW=VariablesGlobales.T+1;
	double minZ=0;
	...
	public ModelePlacementGLPKV7(Cloud cloud,Cout cout){
		////////////////////////
		this.cout=cout;
		this.cloud=cloud;
	...
	public void resoudre(){
		...
		try{
            ...
            //for(int b=0;b<nbReduceSlots;b++){
        		GLPK.glp_add_cols(lp, 1);
        		GLPK.glp_set_col_name(lp, index, "W");
   			 	GLPK.glp_set_col_kind(lp, index, GLPKConstants.GLP_IV);
   			    GLPK.glp_set_col_bnds(lp, index, GLPKConstants.GLP_DB, this.minVW, this.maxVW);
   			 	index++;
            //}
										...
										if(i!=j){
											if(Q[j][i]>0){
												ind = GLPK.new_intArray(4);
			    	  							...
			    	  							GLPK.glp_set_row_bnds(lp, contIndex, GLPKConstants.GLP_UP,Q[j][i], Q[j][i]);
			    	  						    GLPK.intArray_setitem(ind, 1, getXIndex(i,m,a));
			    	  						    GLPK.intArray_setitem(ind, 2, getYIndex(j,r,b));
			    	  						    GLPK.intArray_setitem(ind, 3, getZIndex(b,a));
			    	  						    GLPK.doubleArray_setitem(val, 1, Q[j][i]);
			    	  						    GLPK.doubleArray_setitem(val, 2, Q[j][i]);
			...
			for(int b=0;b<nbReduceSlots;b++){
        		...
        		GLPK.intArray_setitem(ind, ii, getWIndex());
	        	GLPK.doubleArray_setitem(val, ii, -1);
			...
			//}
			//for(int b=0;b<nbReduceSlots;b++){
				GLPK.glp_set_obj_coef(lp, getWIndex(),this.coefRepartition);
			//}
	...
	public void afficher(glp_prob lp){
			...
			for(int m=0;m<nbMapTasks[i];m++){
				for(int j=0;j<nbJobs;j++){
					for(int r=0;r<nbReduceTasks[j];r++){
						for(int a=0;a<nbMapSlots;a++){
							for(int b=0;b<nbReduceSlots;b++){
								if(GLPK.glp_mip_col_val(lp, getXIndex(i,m,a))==1 && GLPK.glp_mip_col_val(lp, getYIndex(j,r,b))==1){
									if(i!=j){
										coutTotalCommunication+=DIST[r][m]*Q[j][i];
									}
									else{
										coutTotalCommunication+=DIST[m][r]*Q[i][j];
									}
								}
							}
						}
		...
		this.cout.coutStockage=coutTotalStockage;
	}
